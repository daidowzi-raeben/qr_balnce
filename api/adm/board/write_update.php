...
if (isset($_POST['wr_subject'])) {
    $wr_subject = mb_substr(trim($_POST['wr_subject']),0,255);
...
@include_once('./'.$board['bo_skin'].'/write_update.head.skin.php');
...
if ($w == '' || $w == 'r') {
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            alert('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.', $return_url);
        else if($member['mb_9'] == '1' &&  $member['mb_10'] == $config['site_code']) { }
...
// 사용자 코드 실행
@include_once('./'.$board['bo_skin'].'/write_update.skin.php');
@include_once('./'.$board['bo_skin'].'/write_update.tail.skin.php');
