...
if ($sca || $stx || $stx === '0') {     //검색이면
    ...
    // 지점것만 보에게 하기 위해
    // $sql_search .= " and ". getBoardAddSearch() . getBoardAddSearchAll();
...
if ($is_search_bbs) {
    ...
    // $sql = " select * from {$write_table} where wr_is_comment = 0 and ". getBoardAddSearch() . ;
    if(!empty($notice_array)) {
...
if($bo_table == "p_campaign")
	include_once('./p_campaign/list.skin.php');
else
	//include_once('./'.$board['bo_skin'].'/list.skin.php');
	include_once('./basic/list.skin.php');