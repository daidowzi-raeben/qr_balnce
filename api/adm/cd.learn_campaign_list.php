...
$L = 'LS03';
$sql_cnt = " SELECT COUNT(lssn_no) AS cnt from cd_lms_lesson WHERE lssn_kind = '".$L."' ";
$result_cnt = sql_fetch($sql_cnt);

$colspan = $result_cnt['cnt'];
?>
...
<!-- 				<th scope="col" id="mb_list_id" rowspan="2">마일리지 합계</th> -->
			...
			$sql_list = "SELECT * from cd_lms_lesson WHERE lssn_kind = '".$L."' and lssn_company = '".$_GET['company']."' ORDER BY lssn_no asc";
			$result_list = sql_query($sql_list);
			for ($i=0; $row=sql_fetch_array($result_list); $i++) {
				?>
				<th scope="col" id="mb_list_name"><?php echo $i + 1?></th>
			<?php }?>
			</tr>
            </thead>
            <tbody>
            ...
            <tr class="<?php echo $bg; ?>">
				...
				for ($u=0; $row3=sql_fetch_array($result_list); $u++) { 
					$sql_is = " SELECT * from cd_lms_lesson_result WHERE mb_id = '".$mb_id."' AND lssn_no = '".$row3['lssn_no']."' and end_dt is not null limit 1 ";
					$result_is = sql_fetch($sql_is);
					?>
				<td headers="cb_list_"><?php if(isset($result_is['mb_id'])) {echo '1'; }else {echo '0';}?></td>
				<?php }?>
...
function selectLocation(v) {
	console.log(v.value)
		location.href='./cd.learn_campaign_list.php?company='+v.value;
}
