...
$sql_common = " from cd_co_cate3 as m where (1) ";
...
<div class="local_ov02">
	...
	<div class="r_div">
		...
		<a href="./co_cat_w3.php" class="btn btn_03">등록</a>
	</div>
</div>
<div>
    ...
    <div class="tbl_head01 tbl_wrap">
		...
		<input type="hidden" name="re" value="./co_cat3.php">
		<input type="hidden" name="table" value="cd_co_cate3">
        <table>
...
FROM cd_co_cate3 AS a WHERE idx = {$row['idx']}";
			$result_latest2 = sql_fetch($sql_latest2);

			$sql_latest3 = "select * from cd_co_cate where idx = {$result_latest2['cate_idx']}";
			$result_latest3 = sql_fetch($sql_latest3);	
			echo $result_latest3['cd_name'];
			?>
				</td>
				<td>
					<?php 
			$sql_latest = "select * from cd_co_cate2 where idx = {$row['cd_idx']}";
			$result_latest = sql_fetch($sql_latest);	
			echo $result_latest['cd_name'];
			?>
				</td>
				<td><a href="./co_cat_w3.php?idx=<?php echo $row['idx']?>&w=u"><?php echo $row['cd_name']?></a></td>
			...
			echo $result_latest3['cd_code']; ?>-<?php echo $result_latest['cd_code']?>-<?php echo $row['cd_code']?></td>
			</tr>
            <?php 
				$startNum++;
			} ?>
