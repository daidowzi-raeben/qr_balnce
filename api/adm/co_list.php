...
$sql_common = " from cd_co_cate4 as m where (1) ";
...
<div class="local_ov02">
	...
	<div class="r_div">
		...
		<a href="./co_list_w.php"  id="member_add" class="btn btn_02">등록</a>
	</div>
</div>
<div>
	...
	<form method="post" action="./co_del.php" id="frm_del">
		<input type="hidden" name="re" value="./co_list.php">
		<input type="hidden" name="table" value="cd_co_cate4">
        <table>
...
$result_latest2 = sql_fetch($sql_latest2);	
echo $result_latest2['cd_name']?></td>
				<td><a href="./co_list_w.php?idx=<?php echo $row['idx']?>&w=u"><?php echo $row['cd_name']?></a></td>
				<td><?php echo $row['cd_dev_date']?></td>
				<td><?php echo $row['cd_time']?></td>
				<td><?php echo $row['cd_file']?></td>
				<td><?php echo $row['cd_connect']?></td>
			</tr>
            <?php 
				$startNum++;
			} ?>
