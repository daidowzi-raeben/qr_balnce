...
if ($w == '') {
    if ($wr_id) {
        alert('글쓰기에는 \$wr_id 값을 사용하지 않습니다.', G5_ADM_BBS_URL.'/board.php?bo_table='.$bo_table);
    }
    ...
    if ($member['mb_level'] < $board['bo_write_level']) {
        if ($member['mb_id']) {
            ...
            alert("글을 쓸 권한이 없습니다.\\n회원이시라면 로그인 후 이용해 보십시오.", G5_ADM_BBS_URL.'/login.php?'.$qstr.'&amp;url='.urlencode($_SERVER['SCRIPT_NAME'].'?bo_table='.$bo_table));
        }
    }
...
$action_url = https_url(G5_BBS_DIR_ADM)."/write_update.php";
